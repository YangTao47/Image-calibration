	...
	const vector<Point2f> &points1_up,const vector<Point2f> &points2_up,string model, float threshold, vector<bool> &inliers, float &rmse)
{
	...
	for (size_t i = 0; i < iterations; ++i)
	{
		...
		while (1)
		{
			randu(rand_mat, 0, double(N - 1));//随机生成n个范围在[0,N-1]之间的数  TODO 没有考虑x与y坐标都重合的时候
			int a = 1;
			//保证这n个点坐标不相同
			if (n == 2 && p[0] != p[1] &&
				(p10[p[0]] != p10[p[1]] || p11[p[0]] != p11[p[1]]) &&
		...
		if (model == string("perspective"))
		{
			...
			Mat match2_xy_change_12 = match2_xy_change(Range(0, 2), Range::all());
			float *temp_ptr = match2_xy_change.ptr<float>(2);// x y z  z平面
			for (size_t i = 0; i < M; ++i)
			{
				float div_temp = temp_ptr[i];
				match2_xy_change_12.at<float>(0, (int)i) = match2_xy_change_12.at<float>(0, (int)i) / div_temp; // x  y 都除以Z平面系数
				match2_xy_change_12.at<float>(1, (int)i) = match2_xy_change_12.at<float>(1, (int)i) / div_temp;
			}
 ...
 */
Mat image_fusion(const Mat &image_1, const Mat &image_2, const Mat T, Mat &fusion_image, Mat &mosaic_image)
{
	...
	warpPerspective(image_2_temp, trans_2, T_1, Size(cols_1, rows_1), 3);
	return trans_2;
	...
	imwrite("D:\\Image-Registration-master\\image_save\\参考图像棋盘图像.jpg", chessboard_1);
	imwrite("D:\\Image-Registration-master\\image_save\\待配准图像棋盘图像.jpg", chessboard_2);
	imwrite("D:\\Image-Registration-master\\image_save\\配准后的待配准图像.jpg", matched_image);*/
}
...
bool sortForPoints(KeyPoint a, KeyPoint b)
{
	if (a.pt.x == b.pt.x) return a.pt.y < b.pt.y;
	else return a.pt.x < b.pt.x;
}

bool equalForPoints(KeyPoint a, KeyPoint b)
{
	return a.pt.y == b.pt.y;
}

void eraseForPoints(vector<KeyPoint> &keys)
{
	sort(keys.begin(), keys.end(), sortForPoints);
	vector<KeyPoint>::iterator new_end;
	new_end = unique(keys.begin(), keys.end(), equalForPoints);
...
{
	vector<KeyPoint> copyKey1 = keys_1;
	vector<KeyPoint> copyKey2 = keys_2;
	//eraseForPoints(keys_1);
	//eraseForPoints(keys_2);
	//获取初始匹配的关键点的位置
	...
	drawMatches(image_1, keys_1, image_2, keys_2, low_matchs,
		...
		vector<char>(), 2);
	imwrite("D:\\Image-Registration-master\\image_save\\初始匹配点对.jpg", initial_matched);

	//绘制正确匹配点对连线图
	drawMatches(image_1, copyKey1, image_2, copyKey2, right_matchs,
		matched_line, Scalar(0, 255, 0), Scalar(255, 0, 0),
		vector<char>(), 2);
	//
	//imwrite("D:\\Image-Registration-master\\image_save\\正确匹配点对.jpg", matched_line);
	...
	imwrite("D:\\Image-Registration-master\\image_save\\参考图像检测到的特征点.jpg", keys_image_1);
	imwrite("D:\\Image-Registration-master\\image_save\\待配准图像检测到的.jpg", keys_image_2);*/
